...
export const listCattle = asyncHandler(async (req, res) => {
  ...
  let rows = await prisma.cattle.findMany({
    ...
    include: { vaccinations: { orderBy: { nextDueDate: "asc" }, take: 1 } },
  });
  ...
  const data = rows.slice(skip, skip + take).map(({ vaccinations, ...cow }) => ({
    ...cow,
...
export const getCattle = asyncHandler(async (req, res) => {
  ...
  const cattle = await prisma.cattle.findFirst({
    ...
    include: {
      vaccinations: { orderBy: { doseDate: "desc" }, take: 6 },
      thermalReadings: { orderBy: { capturedAt: "desc" }, take: 12 },
      gestaChecks: { orderBy: { examDate: "desc" }, take: 6 },
      muzzleProfiles: { orderBy: { createdAt: "desc" } },
